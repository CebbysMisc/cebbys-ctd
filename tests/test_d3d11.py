...
def test_d3d11_enum_members() -> None:
    ...
    print(f"✓ Total typedefs: {len(collection.typedefs)}")
    print(f"✓ Total enums: {len(collection.enums)}")
    ...
    print(f"✓ DriverType enum found with {len(driver_type.members)} members")
    ...
    print(f"✓ Base type resolved: {base_type_ref.target.qualified_name}")
    ...
    print(f"\n✓ HResult typedef found: {hresult_typedef.qualified_name}")
    
    print("\n✓ All enum members auto-incremented correctly")
    print(f"✓ Cross-file type resolution working (d3d11 -> std-types)")
    print(f"✓ New syntax parsed successfully (structure, function, annotation)")

if __name__ == '__main__':
    test_d3d11_enum_members()
    print("\n✅ Test complete!")
