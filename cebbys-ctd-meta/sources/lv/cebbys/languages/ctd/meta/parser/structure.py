...
class CtdStructureContextParser(Api.CtdDeclaractionContextParserBase[Api.CtdParser.StructureDeclarationContext, Meta.StructureMeta]):
    @staticmethod
    def instance() -> Api.CtdDeclaractionContextParser[Api.CtdParser.StructureDeclarationContext, Meta.StructureMeta]:
        return INSTANCE

    def parse(self, namespace: str, ctx: Api.CtdParser.StructureDeclarationContext) -> Meta.StructureMeta:
        ...
        member_list_ctx = self.optional_rule(ctx, Api.CtdParser.StructureMemberListContext)
        if member_list_ctx:
            for member_ctx in self.rules(member_list_ctx, Api.CtdParser.StructureMemberContext):
                member_name = self.text(self.token(member_ctx, Api.CtdParser.IDENTIFIER))
                member_typespec_ctx = self.rule(member_ctx, Api.CtdParser.TypeSpecContext)
        ...
        return Meta.StructureMeta(name, namespace, base_type, members)
